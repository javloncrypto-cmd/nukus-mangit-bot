...
from keyboards.keyboards import (
    ...
    main_menu_kb, passenger_confirm_kb, complaint_kb,
)
...
async def got_location(message: Message, state: FSMContext):
    ...
    await message.answer("👥 Necha kishi? (o'zingiz bilan birga):", reply_markup=passengers_count_kb())
...
async def got_count(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("💰 Taklif narxingizni yozing (masalan: 25000):", reply_markup=cancel_kb())
...
async def got_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📝 Izoh qo'shing (ixtiyoriy). Masalan: 'Mushugim bor'\nYo'q bo'lsa /skip yozing:",
