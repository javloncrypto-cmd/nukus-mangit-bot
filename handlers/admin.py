...
async def blacklist(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if not low_rating:
        text = "Hozircha reytingi past haydovchi yo'q.\n\n"
    else:
        text = "🚫 <b>Past reytingli haydovchilar:</b>\n\n"
        for driver_id, avg_score in low_rating:
            user = await queries.get_user(session, driver_id)
            name = user.full_name if user else "Noma'lum"
            text += f"• {name} <code>{driver_id}</code> — ⭐ {avg_score:.1f}\n"
        text += "\n"

    await message.answer(text, parse_mode="HTML")
    await state.set_state(AdminForm.waiting_ban_id)
    await message.answer("Bloklash uchun foydalanuvchi ID sini yuboring (/cancel bekor qilish):")
...
async def ban_user_action(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await state.clear()
        await message.answer(f"✅ {user.full_name} (<code>{user_id}</code>) bloklandi.", parse_mode="HTML")
    except ValueError:
...
async def unban_user_action(message: Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await state.clear()
        await message.answer(f"✅ {user.full_name} (<code>{user_id}</code>) blokdan chiqarildi.", parse_mode="HTML")
    except ValueError:
...
async def complaint_ban(callback: CallbackQuery, session: AsyncSession, bot: Bot):
    if not await check_admin(session, callback.from_user.id):
        ...
        return
    complaint_id = int(callback.data.split("_")[2])
    complaint = await session.get(type(None), complaint_id)  # we'll fetch manually
    ...
    try:
        await bot.send_message(c.against_user_id, "🚫 Siz shikoyat asosida bloklandingiz. Admin bilan bog'laning.")
    except Exception:
        pass
